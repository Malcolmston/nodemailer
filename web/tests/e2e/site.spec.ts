  ...
  for (const id of TAB_IDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        // /nodemailer/api/. It is produced by the Go doc generator at deploy time,
        ...
        expect(resolved.pathname, XPLACEHOLDER;
      } else {
...
test('docs tab renders the React API reference with a package listed', async ({ page }) => {
  ...
  // nodemailer is a single-package module (package nodemailer), so at least one package link is expected.
